...
def getUserSavedSongsData():
    ...
    userSavedSongNames = list(trackNames)
...
def getAudioFeatures(savedURIs, artistName):
    ...
    print('\nGathering audio data from ' + str(iterations) + ' tracks...')
...
def recommendSongs(artistName, favSongs):
    ...
    for x in range(len(favSongClusters)):
        ...
        for y in range(len(recSongs)):
            ...
            if songName.lower() not in favSongsLowercase and userFavSong.casefold() not in songName.casefold() and userFavSongCluster == songCluster and songName not in outputSongs and recSongs[y][3] not in userSavedSongURIs:
                
                if artistName == 'recSongs' and recSongArtistName.casefold() != artist.casefold(): 
                    print(recSongArtistName, songName)

                elif artistName != 'recSongs' and songName not in userSavedSongNames:
                    print(songName)
