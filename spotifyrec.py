...
def recommendSongs(artist, favSongs):
    ...
    for x in range(len(data)):
        ...
        for song in favSongs:
            if clean(song.lower()) in clean(trackName):
                if data[x][3] not in clusters:
                    clusters.append(data[x][3])
    ...
    for x in range(len(data)):
        ...
        if cluster in clusters:
            recSongs.append(trackName[1:])


    for rs in recSongs:
        for song in favSongs:
            if song.casefold() in rs.casefold():
                recSongs.remove(rs)


    recSongs = list(set(recSongs)) 
