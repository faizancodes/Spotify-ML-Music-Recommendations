...
def clusterData(fileName, artist):
    ...
    if artist == 'recSongs':
        ...
        addedClusters = int(len(dataset) / 30)
...
def recommendSongs(artistName, favSongs):
    ...
    for x in range(len(favSongClusters)):
        ...
        for y in range(len(recSongs)):
            ...
            if songName.lower() not in favSongsLowercase and recSongArtistName not in topArtists and userFavSong.casefold() not in songName.casefold() and userFavSongCluster == songCluster and songName not in outputSongs:
