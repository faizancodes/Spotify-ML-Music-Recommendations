...
def recommendSongs(artist, favSongs):
    ...
    for x in range(len(favSongClusters)):
        ...
        for y in range(len(recSongs)):
            ...
            if favSongClusters[x][0].casefold() not in songName.casefold() and favSongClusters[x][1] == songCluster and recSongs[y][0] not in outputSongs:
                
                print(recSongs[y][0])
                outputSongs.append(recSongs[y][0])

    
    print('\nTotal Songs:', len(recSongs) - len(favSongs))
